...
export default {
  ...
  methods: {
    getRestaurateurs(restaurateur_page) {
      (this.loading = true), console.log(store.selectedType);
      axios
        .get(`${this.store.baseUrl}/api/restaurateurs`, {
          params: { page: restaurateur_page },
        })
        .then((response) => {
    ...
    <div class="row">
      <div class="parent">
        ...
        <div v-else class="row-grid">
          <div
            v-for="restaurateur in restaurateur"
            class="my-3"
            :key="restaurateur.id"
          >
      ...
      <div class="col-12 d-flex justify-content-center">
        <nav>
          <ul class="d-flex list-unstyled gap-2">
            <li
              :class="this.store.currentPage === 1 ? 'disabled' : 'page-item'"
            >
              <button
                class="page-link"
                @click="getRestaurateurs(this.store.currentPage - 1)"
              >
                <i class="fa-solid fa-angle-left"></i> Prev
              </button>
            </li>
            <li
              :class="this.store.currentPage === i ? 'disabled' : 'page-item'"
              v-for="i in this.store.lastPage"
            >
              <button class="page-link" @click="getRestaurateurs(i)">
                {{ i }}
              </button>
            </li>
            <li
        ...
        <nav>
          <ul class="d-flex list-unstyled gap-2">
            <li :class="this.currentPage === 1 ? 'disabled' : 'page-item'">
              <button
                class="page-link"
                @click="getRestaurateurs(this.currentPage - 1)"
              >
                <i class="fa-solid fa-angle-left"></i> Prev
              </button>
            </li>
            <li
              :class="this.currentPage === i ? 'disabled' : 'page-item'"
              v-for="i in this.lastPage"
            >
              <button class="page-link" @click="getRestaurateurs(i)">
                {{ i }}
              </button>
            ...
            <li
              :class="
                this.currentPage === this.lastPage ? 'disabled' : 'page-item'
              "
            >
              <button
                class="page-link"
  ...
  <div class="container" v-else>
    <div class="row">
      <div class="parent">
        ...
        <div v-else class="row-grid">
          <div
            v-for="restaurateur in this.store.restaurateurs"
            class="my-3"
            :key="restaurateur.id"
          >
            <RestaurateurCard :restaurateur="restaurateur" />
